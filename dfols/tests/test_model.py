...
class TestAddValues(unittest.TestCase):
    def runTest(self):
        ...
        model = Model(npt, x0, rosenbrock(x0), h, xl, xu, [], 1)
...
class TestRegression(unittest.TestCase):
    def runTest(self):
        ...
        xu = 1e2 * np.ones((n,))
        h = lambda d: np.linalg.norm(d, 1)
        model = Model(npt, A[0,:], np.array([b[0]]), h, xl, xu, [], 1)
...
class TestUnderdetermined(unittest.TestCase):
    def runTest(self):
        ...
        h = lambda d, *argh: np.linalg.norm(d, 1)
        model = Model(n+1, A[0, :], np.array([b[0]]), h, xl, xu, [], 1)
